...
impl RoomServerAclEventContent {
    ...
    /// Returns true if the input matches a wildcard pattern in the allow list.
    #[inline]
    pub fn allow_matches(&self, host: &str) -> bool {
        Self::matches(&self.allow, host)
    }

    /// Returns true if the input matches a wildcard pattern in the deny list.
    #[inline]
    pub fn deny_matches(&self, host: &str) -> bool {
        Self::matches(&self.deny, host)
    }

    /// Returns true if the input is equal to a string in the allow list.
    #[inline]
    pub fn allow_contains(&self, host: &str) -> bool {
        Self::contains(&self.allow, host)
    }

    /// Returns true if the input is equal to a string in the deny list.
    #[inline]
    ...
    fn matches(a: &[String], s: &str) -> bool {
        a.iter().any(|p| WildMatch::new_case_insensitive(p).matches(s))
    }

    fn contains(a: &[String], s: &str) -> bool {
        a.iter().any(|p| p.eq_ignore_ascii_case(s))
    }
}
